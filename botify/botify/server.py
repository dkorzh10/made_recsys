...
from botify.recommenders.our_recommender import OurRecommender
...
tracks_redis_ours = Redis(app, config_prefix="REDIS_TRACKS_OURS")
...
catalog_our = Catalog(app).load(
    app.config["OUR_TRACKS_CATALOG"], app.config["TOP_TRACKS_CATALOG"]
)
catalog_our.upload_tracks(tracks_redis_ours.connection)
catalog_our.upload_artists(artists_redis.connection)
catalog_our.upload_recommendations(recommendations_redis.connection)
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        treatment = Experiments.OUR_RECOM.assign(user)
        if treatment == Treatment.T1:
            recommender = Contextual(tracks_redis_ours.connection, catalog_our)
        else:
